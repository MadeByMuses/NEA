...
async function PurchasePolicyPack(id){
    ...
    if (GetDBElements("City_Attribute","attribute_value","city_attribute_id",4) < GetDBElements("Policy_Pack","policy_pack_cost","policy_pack_id",id)) {
        ...
        document.getElementById("ConfirmationContent").innerHTML = `<img style="width:50%;image-rendering: pixelated;" src="Assets/Transaction.gif"><br><ul id="PolicyPackPurchaseWhatDidYouGet"></ul>`;
        await delay(1000);
        for (n of GetDBElements("Policy_Pack","policies_to_gain","policy_pack_id",id)){
            //document.ptElementById("PolicyPackPurchaseWhatDidYouGet").
            await delay(500);
        }
        await delay(500);
        document.getElementById("ConfirmationContent").innerHTML = `<h2>You have purchased:<br>` + GetDBElements("Policy_Pack","policy_pack_name","policy_pack_id",id) + `</h2><hr><p>You may now close this window, thank you for your patronage`;
    }
}
